...
$string['phpversion'] = 'Academe ePortfolios will not run on PHP < %s. Please upgrade your PHP version or move Academe ePortfolios to a different host.';
$string['jsonextensionnotloaded'] = 'Your server configuration does not include the JSON extension. Academe ePortfolios requires this in order to send some data to and from the browser. Please make sure that it is loaded in php.ini or install it if it is not installed.';
$string['pgsqldbextensionnotloaded'] = 'Your server configuration does not include the pgsql extension. Academe ePortfolios requires this in order to store data in a relational database. Please make sure that it is loaded in php.ini or install it if it is not installed.';
$string['mysqldbextensionnotloaded'] = 'Your server configuration does not include the mysqli or mysql extension. Academe ePortfolios requires this in order to store data in a relational database. Please make sure that it is loaded in php.ini or install it if it is not installed.';
$string['mysqlmodulenolongersupported1'] = 'Your server configuration does not include the mysqli extension. Please make sure that it is loaded in php.ini or install it if it is not installed. Academe ePortfolios stopped supporting the mysql extension in version 16.10';
...
$string['domextensionnotloaded'] = 'Your server configuration does not include the dom extension. Academe ePortfolios requires this in order to parse XML data from a variety of sources.';
$string['mbstringextensionnotloaded'] = 'Your server configuration does not include the mbstring extension. Academe ePortfolios requires this to parse multi-byte strings for varying languages.';
$string['xmlextensionnotloaded'] = 'Your server configuration does not include the %s extension. Academe ePortfolios requires this in order to parse XML data from a variety of sources. Please make sure that it is loaded in php.ini or install it if it is not installed.';
$string['gdextensionnotloaded'] = 'Your server configuration does not include the gd extension. Academe ePortfolios requires this in order to perform resizes and other operations on uploaded images. Please make sure that it is loaded in php.ini or install it if it is not installed.';
...
$string['sessionextensionnotloaded'] = 'Your server configuration does not include the session extension. Academe ePortfolios requires this in order to support people logging in. Please make sure that it is loaded in php.ini or install it if it is not installed.';
$string['curllibrarynotinstalled'] = 'Your server configuration does not include the cURL extension. Academe ePortfolios requires this for Moodle integration and to retrieve external feeds. Please make sure that cURL is loaded in php.ini or install it if it is not installed.';
$string['intlextensionnotloaded'] = 'Your server configuration does not include the intl extension for PHP. Academe ePortfolios requires this extension to parse information related to language locales.';
$string['registerglobals'] = 'You have dangerous PHP settings: register_globals is on. Academe ePortfolios is trying to work around this, but you should really fix it. If you are using shared hosting and your host allows for it, you should include the following line in your .htaccess file:
php_flag register_globals off';
$string['magicquotesgpc'] = 'You have dangerous PHP settings: magic_quotes_gpc is on. Academe ePortfolios is trying to work around this, but you should really fix it. If you are using shared hosting and your host allows for it, you should include the following line in your .htaccess file:
php_flag magic_quotes_gpc off';
$string['magicquotesruntime'] = 'You have dangerous PHP settings: magic_quotes_runtime is on. Academe ePortfolios is trying to work around this, but you should really fix it. If you are using shared hosting and your host allows for it, you should include the following line in your .htaccess file:
php_flag magic_quotes_runtime off';
$string['magicquotessybase'] = 'You have dangerous PHP settings: magic_quotes_sybase is on. Academe ePortfolios is trying to work around this, but you should really fix it. If you are using shared hosting and your host allows for it, you should include the following line in your .htaccess file:
php_flag magic_quotes_sybase off';

$string['safemodeon'] = 'Your server appears to be running safe mode. Academe ePortfolios does not support running in safe mode. You must turn this off in either the php.ini file or in your apache config for the site.
...
$string['apcstatoff'] = 'Your server appears to be running APC with apc.stat=0. Academe ePortfolios does not support this configuration. You must set apc.stat=1 in the php.ini file.
...
$string['couldnotmakedatadirectories'] = 'For some reason some of the core data directories could not be created. This should not happen as Academe ePortfolios previously detected that the dataroot directory was writable. Please check the permissions on the dataroot directory.';

$string['dbconnfailed'] = 'Academe ePortfolios could not connect to the application database.

 * If you are using Academe ePortfolios, please wait a minute and try again
 * If you are the administrator, please check your database settings and make sure your database is available

The error received was:
';
$string['dbnotutf8'] = 'You are not using a UTF-8 database. Academe ePortfolios stores all data as UTF-8 internally. Please drop and re-create your database using UTF-8 encoding.';
$string['dbnotutf8mb4'] = 'You are not using a utf8mb4 Character Set (4-Byte UTF-8 Unicode Encoding) database. Academe ePortfolios stores all data as utf8mb4 internally. Please drop and re-create your database using utf8mb4 encoding.';
$string['dbversioncheckfailed'] = 'Your database server version is not new enough to successfully run Academe ePortfolios. Your server is %s %s, but Academe ePortfolios requires at least version %s.';
$string['plpgsqlnotavailable'] = 'The PL/pgSQL language is not enabled in your Postgres installation, and Academe ePortfolios cannot enable it. Please install PL/pgSQL in your database manually. For instructions on how to do this, see https://wiki.mahara.org/wiki/System_Administrator\'s_Guide/Enabling_Plpgsql';
$string['mysqlnotriggerprivilege'] = 'Academe ePortfolios requires permission to create database triggers, but is unable to do so. Please ensure that the trigger privilege has been granted to the appropriate user in your MySQL installation. For instructions on how to do this, see https://wiki.mahara.org/wiki/System_Administrator\'s_Guide/Granting_Trigger_Privilege';
...
$string['cssnotpresent'] = 'CSS files are not present in your htdocs/theme/raw/style directory. If you are running Academe ePortfolios from a git checkout, run "make css" to build the CSS files. If you are running Academe ePortfolios from a ZIP download, try downloading and unzipping again.';
$string['mahararootusermissing'] = 'The "root" account is missing from the database so we cannot continue. This account needs to be present for Academe ePortfolios to function correctly. To add the root account back in, please make another install of the Academe ePortfolios version you are using and see what is contained for account id = 0 in the "usr" and "usr_custom_layout" tables and add that data to your instance of Academe ePortfolios before trying to upgrade again.';
...
$string['switchtomysqli'] = 'The <strong>mysqli</strong> PHP extension is not installed on your server. Thus, Academe ePortfolios is falling back to the deprecated original <strong>mysql</strong> PHP extension. We recommend installing <a href="http://php.net/manual/en/book.mysqli.php">mysqli</a>.';
...
$string['openbasedirpaths'] = 'Academe ePortfolios can only open files within the following path(s): %s.';
...
$string['wrongsessionhandle'] = 'The session save handler "%s" is not supported in Academe ePortfolios.';
